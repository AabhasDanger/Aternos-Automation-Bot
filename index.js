...
client.on('message',(message)=>{
    ...
    if(message.author.bot == true){
        ...
        if(channel.id==env.CONTROLCHANNEL){
                    ...
                    if(currentServers.includes(splited[1])==true){
                        ...
                        if(splited[2]=='ip'){
                            ...
                            (async()=>{
                                ...
                                await page.click('[data-id="'+dataid+'"]')
                                await page.waitForSelector('[class="statuslabel-label"]')
                                if((await page.$eval('[class="statuslabel-label"]',el => el.innerText))!='Offline'){
                                    let embed = new Discord.MessageEmbed()
                                        .setColor('#ff1a1a')
                                        .setTitle('Error!')
                                        .setDescription(`This error can be because 
                                        - The Server is already on
                                        -The Server is in Loading stages
                                        -The Server is in que`)
                                    message.channel.send(embed)
                                }else{
                                    
                                    let button = await page.evaluateHandle(()=>document.querySelector('#start'))
                                    setTimeout(async()=>{
                                        button.click()
                                        setTimeout(async()=>{
                                            let currentStatus = await page.$eval('[class="statuslabel-label"]',el => el.innerText)
                                            if(currentStatus=='Preparing ...'||currentStatus=='Loading ...'){
                                                let PreparingEmbed = new Discord.MessageEmbed()
                                                    .setTitle('Preparing...')
                                                    .setDescription('The Server has successfully started and is now on the preparing stage.')
                                                message.channel.send(PreparingEmbed)
                                                browser.close()
                                            }else if(currentStatus=='Waiting in queue'){
                                                page.click('[onclick="requestnotification()"]')
                                                let queTime = await page.$eval('[class="server-status-label-left queue-time hidden"]',el => el.innerText)
                                                let Embed = new Discord.MessageEmbed()
                                                    .setColor('#ff8000')
                                                    .setTitle('Waiting in Que')
                                                    .setDescription(`The Que will take `+'`'+queTime+'`'+` at estimate`)
                                                message.channel.send(Embed)
                                                let actualQue = (parseInt(queTime.split(' ')[1])*60000)-70000
                                                let confirmLoop = setInterval(async()=>{
                                                    let dis = await page.evaluate(()=>window.getComputedStyle(document.querySelector('#confirm')).display)
                                                    if(dis!='none'){
                                                        page.click('#confirm')
                                                        browser.close()
